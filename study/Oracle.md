...
---
#### 실행순서 <br/>
: FROM  ->  ON  ->  JOIN  ->  WHERE ->  GROUP BY  ->  CUBE + ROLLUP -> HAVING ->  SELECT  ->  DISTINCT  ->  ORDER BY  -> TOP <br/>
...
## 데이터 타입
: 컬럼이 저장되는 데이터 유형.
- 종류 : 원시 데이터 타입(오라클에서 기본으로 제공), 사용자 정의 타입
* 문자형
  - CHAR(크기[BYTE|CHAR]) : 고정 길이 문자. 최대 2000byte, 디폴트 1byte.
  - VARCHAR2(크기[BYTE|CHAR]) : 가변 길이 문자. 최대 40000byte, 디폴트 1byte.
  - NCHAR(크기) : 고정길이 유니코드 문자(다국어 입력가능). 최대 2000byte, 디폴트 1byte.
  - NVARCHAR2(크기) : 가변길이 유니코드 문자(다국어 입력가능). 최대 4000byte, 디폴트 1byte.
  - LONG : 가변길이 문자. 최대 2GB.
* 숫자형
  - NUMBER 
  - FLOAT
...
<br/><br/>
## 도메인Domain
: 데이터 표준화를 위해 동일한 테이블 타입을 가지는 속성을 분리하는 것.
* 데이터 타입, 크기, 제약조건, 성질을 통일시키는 역할. 일관성과 정합성 유지.
* 타입, 자릿수, 포멧 등이 동일 (유사 속성에 동일한 데이터 타입 할당)
* 용어(속성)의 뒤에 붙는 단어와 같음 ~> 가격, 금액, 내용, 수, 명, 일자 등
* 데이터 품질 향상
* 속성(용어) =/= 도메인(단어)
* 구성 요소 : Unique Identifier, 일반 Attribute, Code
* 


<br/><br/>
## 인덱스Index
* B-tree 인덱스 : 관계형 데이터베이스에서 가장 일반적으로 사용.
  - 구성 : Root block, Branch block, Leaf block
